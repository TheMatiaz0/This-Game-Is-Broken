...
namespace Lean.Localization
{
	...
	public class LeanLocalization_Inspector : LeanInspector<LeanLocalization>
	{
		...
		private void DrawLanguages()
		{
			...
			{
				EditorGUILayout.HelpBox("Click the 'Add' button, and select a language.", UnityEditor.MessageType.Info);
			}
		...
		private void DrawTranslations()
		{
			...
			{
				EditorGUILayout.HelpBox("Type in the name of a translation, and click the 'Add' button. Or, drag and drop a prefab that contains some.", UnityEditor.MessageType.Info);
			}
			else
			{
				var total = 0;
					...
					foreach (var pair in LeanLocalization.CurrentTranslations)
					{
						...
						if (string.IsNullOrEmpty(translationFilter) == true || name.IndexOf(translationFilter, System.StringComparison.InvariantCultureIgnoreCase) >= 0)
						{
							...
							if (expand == true)
							{
								...
								{
									EditorGUILayout.HelpBox("This translation isn't defined for the " + language + " language.", UnityEditor.MessageType.Warning);
								}

								foreach (var language in clashes)
								{
									EditorGUILayout.HelpBox("This translation is defined multiple times for the " + language + " language.", UnityEditor.MessageType.Warning);
								}
							}
						}
					}
				...
				if (total == 0)
				{
					EditorGUILayout.HelpBox("No translation with this name exists, click the 'Add' button to create it.", UnityEditor.MessageType.Info);
				}
			}
		}
		...
		private void DrawTokens()
		{
			...
			if (LeanLocalization.CurrentTokens.Count > 0 || string.IsNullOrEmpty(tokensFilter) == false)
			{
				...
				if (total == 0)
				{
					EditorGUILayout.HelpBox("No token with this name exists, click the 'Add' button to create it.", UnityEditor.MessageType.Info);
				}
			}
		}
